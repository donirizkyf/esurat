...
Dokumen ini khusus untuk super admin atau admin teknis yang mengelola akun petugas dengan jenis akun `admin` dan `super_admin` langsung dari server atau PC lokal, atau sementara melalui form web publik dengan kode registrasi internal.
...
- `admin`
  Digunakan untuk petugas internal operasional dengan hak terbatas.
- `super_admin`
  Digunakan untuk akun utama pengelola sistem.

Fungsi kerja petugas seperti `OA`, `Monitoring`, `PIC`, dan `Staff` disimpan terpisah sebagai label tugas internal, bukan sebagai jenis akun login.
...
Selain lewat CLI, akun petugas juga bisa didaftarkan dari halaman `/register` dengan memilih role `admin` atau `super_admin`, lalu memasukkan kode registrasi internal dari environment variable berikut:

- `PETUGAS_REGISTRATION_CODE` untuk `admin`
...
- role harus salah satu dari `admin` atau `super_admin`
- `account_status` harus `ACTIVE`

Setelah login:

- petugas diarahkan ke dashboard internal
...
- gunakan `admin` untuk petugas operasional harian
- gunakan kolom fungsi kerja untuk menandai apakah petugas adalah `OA`, `Monitoring`, `PIC`, atau `Staff`
...
```powershell
python -m app.manage_users create --username admin2 --password Password123 --role admin --status ACTIVE
```
...
```powershell
python -m app.manage_users set-status --identifier admin2 --status ACTIVE
```
